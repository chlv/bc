		...
		while cline = c_file.gets  
			...
			File.open("C:\\Users\\Administrator\\AppData\\Local\\Temp\\device.txt","r") do |sut_file|
				while sline = sut_file.gets
					if  did != nil and sline.include?("VEN_11AA") and sline.include?(did)  # need to change according to the vendor
						flag = 1
						puts("Device found")
						puts sline
					end
...
#			puts (Dir.getwd())
			...
			if rc == 0
				Dir.chdir(".\\pcietestdriverv2.0.1000.0\\2.0.1000.0")      # need to change according to the driver
				osbit = Check_OS_Bit()
				Dir.chdir(osbit)
